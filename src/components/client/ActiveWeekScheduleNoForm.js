...
export default function ActiveWeekScheduleNoForm({
  ...
  function selectHandler(schedule, appointment) {
    setSelectedSchedule(schedule);
    setSelectedAppointment(
      selectedAppointment === appointment ? null : appointment
    );
  }
              ...
              {Object.keys(selectedDaySchedule?.schedule).map((itemKey) => {
                ...
                return (
                  <li
                    ...
                    onClick={() =>
                      !isDisabled
                        ? selectHandler(selectedDaySchedule, itemKey)
                        : null
                    }
                  >
                    {isReserved && (
                    ...
                    {selectedAppointment === itemKey && (
                      <span className="absolute -top-2 right-0 z-20 w-6 h-6 bg-white rounded-full animate__animated animate__bounceIn">
                        <CheckCircleIcon
                          className={"ml-1 mt-1 w-4 h-4 text-green-500"}
                        />
                      </span>
                    )}
                    <span
